...
def workout_view(request):
    """
    Создание/управление текущей тренировкой.
    Логика таймера — на фронтенде (JS).
    По нажатию "Завершить тренировку" сохраняем данные в Workout и Exercise.
    """
    if request.method == 'POST':
        # Логика сохранения результата тренировки
        # Достаем данные из request.POST
        ...
        # Определяем номер тренировки - условно берем количество тренировок + 1
        existing_workout_count = Workout.objects.filter(user=request.user).count()
        workout_number = existing_workout_count + 1
        ...
        # Теперь обрабатываем упражнения
        exercise_count = int(request.POST.get('exercise_count', 0))
        for i in range(1, exercise_count+1):
            # Для каждого упражнения i с фронта придут поля
            exercise_type_id = request.POST.get(f'exercise_type_{i}', None)
            exercise_weight = request.POST.get(f'exercise_weight_{i}', None)
            # Создаем Exercise
            Exercise.objects.create(
                workout=new_workout,
                exercise_type=ExerciseType.objects.get(id=exercise_type_id) if exercise_type_id else None,
                exercise_number=i,
                exercise_weight=exercise_weight if exercise_weight else None
            )

        return redirect('main')

    # Если GET — рендерим страницу с формой, таймером, JS
    # Список доступных упражнений:
    exercise_types = ExerciseType.objects.all()
    return render(request, 'fitnessapp/workout.html', {'exercise_types': exercise_types})
